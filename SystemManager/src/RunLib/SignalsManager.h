 ...
 */
#ifndef SIGNALSMANAGER_H_
#define SIGNALSMANAGER_H_
...
class SignalsManager {
    ...
    public:
        ...
        static int Install(int nSigNum, sighandler_t p_pSigHandler = 0);

        static int Uninstall(int nSigNum);

        static bool IsInstalled(int nSigNum, sighandler_t p_pSigHandler = 0);

        static int Ignore(int nSigNum);

        static int IsRaised(int nSigNum) {
            return m_pSigVector[nSigNum].m_nRaised;
        }

        static void Raise(int nSigNum) {
            m_pSigVector[nSigNum].m_nRaised = 1;
        }

        static void Reset(int nSigNum) {
