...
check_time()
{
	...
	#using current time to schedule once per hour setting of RTC is faulty because time
	# can be modified in past by ntpd
	#Just transfer the time to RTC once per utils.sh run
...
# in order to reduce the stress level
	...
	check_cgi_upload_files > /dev/null
