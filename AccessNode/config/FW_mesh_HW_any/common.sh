...
stop_waitable()
{	 
	local endUpTime TMP_REZ_FILE=/tmp/123.tmp  
...
VerifyConsistentHwVer()
{
	...
	if [ ! -z "$fs_hw_ver" -a ! -z "$hw_no" ]; then
		if [ "$fs_hw_ver" -ne "$hw_no" ]; then
			...
			echo "Board hw $fs_hw_ver != fw HW $hw_no "	>> /access_node/activity_files/work_time.debug
			return 1
		fi
	fi
	return 0
...
#check_invalid_date - executes the date command, and if the year is less than 2000,
#						try to get date from rtc, twice with 3 sec delay
#                     this function will reset the date to 00:00 Jan 1 2000
check_invalid_date()
{
...
MemoryStatus()
{
	...
	tmpfs_Used=`df | sed -n "s/tmpfs *[0-9]* *\([0-9]*\).*/\1/p"`
