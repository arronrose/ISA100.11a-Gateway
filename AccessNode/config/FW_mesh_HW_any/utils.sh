...
truncate_file()
{
	...
	[ $msg_file_size -gt $MSG_FILE_SIZE_LIMIT ] &&\
		cat /dev/null > $FILE &&\
...
check_login()
{
	...
	truncate_file "/var/log/http.log"
	truncate_file "/var/log/https.log"
...
check_time()
{
	latest=`sed -n "s/${TIME_RTC_VAR}=//p" ${UTILS_FILE}`
	now=`date +%s`
	if [ -z ${latest} ]; then
		latest=${now}
		add_var ${TIME_RTC_VAR} ${now}
	fi

	#set RTC only only at 3600 sec intervals
	if [ $(($now-$latest)) -gt ${TIME_RTC_DFLT_LIMIT} ]; then
		ntpadjust.sh				#update RTC time before any other operation
		update_var ${TIME_RTC_VAR} ${now}
	else
		ntpadjust.sh no_rtc			#update RTC only on Linux time update
	fi
}
