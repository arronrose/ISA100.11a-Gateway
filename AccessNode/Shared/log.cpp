...
$CVSHeader: AccessNode/Shared/log.cpp,v 2.70 2010/03/15 16:13:37 marcel Exp $
***************************************************************************/
#include <fcntl.h>
#include <sys/wait.h>
#include <dirent.h>
...
void	CLog::logMaintenance()
{
	...
	char sFileName[AttachDatetime(m_sFileName)];
	AttachDatetime(m_sFileName,sFileName);

	bool bFtpReq = FileIsExecutable(FTP_SCRIPT);

	if (m_bArchive || bFtpReq)
	{
		if (m_bTailSafe)
