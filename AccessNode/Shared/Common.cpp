...
{
	return GetHex( (const void*)&p_stAddr, sizeof(p_stAddr), ' ' );
}

const char * Id2string( const unsigned int p_unAddr )
{
	return GetHex( (const void*)&p_unAddr, sizeof(p_unAddr), ' ' );
}
...
bool EscapeString(char *p_szString)
{
    ...
    static  esc_char pEscChars[] = {
                {'n','\n'}, {'r','\r'},
                {'f','\f'},{'t','\t'},
                {'b','\b'},{'a','\a'},
                {'v','\v'}, {'\\','\\'},
                };
    int nLen = sizeof( pEscChars )/sizeof(esc_char);

	int nSzSize = strlen(p_szString);
    int i,j,k;

    for( i=0, j=0 ; i < nSzSize; i++,j++ )
    {
        if( p_szString[i] == '\\' )
        {   i++;
			...
			if (p_szString[i] == 'x')
			{	int nTmp;
				if ( sscanf( p_szString + i + 1,"%02x",  &nTmp ) > 0 )
				{	p_szString[j] = (char)nTmp;
					i += 2;
					continue;
				}
			}
			...
			{
				for( k = 0; k < nLen; k++ )
				{   if( p_szString[i] == pEscChars[k].m_cDisplay )
					{   p_szString[j] =  pEscChars[k].m_cValue;
						break;
					}
				}
	            if( k < nLen )
