...
bool cleanExtraVariables( const char * p_szEUI64 )
{
    ...
    int retCode = 0;
    if ( oIniParser.Load( CONF_MHA_PUBLISHERS, "r+", true )) {
        retCode += oIniParser.DeleteVar( p_szEUI64, "AUTO_DETECT", 0 );
        int v_nBurstCount = 8;
        char buff[5] = "";

        if ( oIniParser.GetVar( p_szEUI64, "BURST_NO_TOTAL_CMD_105", buff, 5, 0) && buff)
            v_nBurstCount = atoi(buff);

        for ( int i = 0; i < v_nBurstCount; ++i ) {
            char v_szVarName1[20];
            sprintf( v_szVarName1, "BURST_SET_STAT_%i", i );
            retCode += oIniParser.DeleteVar( p_szEUI64, v_szVarName1, 0 );
            char v_szVarName2[32];
            sprintf( v_szVarName2, "BURST_SET_PUBLISH_PERIOD_%i", i );
            retCode += oIniParser.DeleteVar( p_szEUI64, v_szVarName2, 0 );
        }
        retCode += oIniParser.DeleteVar( p_szEUI64, "BURST_NO_TOTAL_CMD_105", 0 );
    }
    return (retCode == 0);
}
