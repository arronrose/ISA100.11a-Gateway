                              ...
                              , unsigned short  p_usVarPos/* = 0*/)
{
	...
	int retCode = oIniParser.SetVar( p_kszGroup, p_kszVarName, p_kszVarValue, p_usVarPos, true);
	if (retCode == 0)
		return false;
...
char *CConfigImpl::getVariable(const char *p_kszConfigName, const char *p_kszGroup, const char *p_kszVarName, unsigned short p_ushVarPos )
{
	...
	int retCode=oIniParser.GetVar(p_kszGroup, p_kszVarName, szVarValue, MAX_LINE_LEN, p_ushVarPos );
	if (retCode == 0)
	{
		return NULL;
	}
