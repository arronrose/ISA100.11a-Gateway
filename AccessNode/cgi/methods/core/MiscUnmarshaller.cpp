...
int CMiscUnmarshaller::unpackTgz( RPCCommand& cmd )
{
	char *script;

	cmd.outObj = json_object_new_object();

	JSON_GET_MANDATORY_STRING_PTR( "script", script );

	std::vector<char*> outVec ;

	// Invoke actual method
	bool return_value  = m_oMisc.unpackTgz( script, outVec );

	struct json_object* retOutput = json_object_new_array();
	for ( unsigned i=0; i< outVec.size() ; ++i )
	{
		json_object_array_add( retOutput, json_object_new_string(outVec[i]) );
		free(outVec[i]);
	}

	if ( !return_value )
	{
		json_object_object_add(cmd.outObj, "error", json_object_new_string("Unable to unpackTgz")  );
		return false ;
	}
	json_object_object_add(cmd.outObj, "result", retOutput );
	return true ;
}
...
int CMiscUnmarshaller::applyConfigChanges( RPCCommand& cmd )
{
	...
	int nRet = m_oMisc.applyConfigChanges( pModule );
