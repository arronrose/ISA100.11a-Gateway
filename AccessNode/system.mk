...
#    <exename>.CFLAGS
#                         for OS specific or test files, -I.. is added
...
_EXEDIR    := $(realpath $(TOP))/$(_OUTPUT_TUPLE)/$(_MODULE_DIR)
_OBJDIR    := $(_EXEDIR)/objs
_LIBDIR    ?= $(AUX_LIBS_DIR_LIB)
_LIBOUTDIR := $(realpath $(TOP))/$(_OUTPUT_TUPLE)/libs$(_LIBEXT)
...
#####################################################################################
define CMPL_PROGRAM
tests clean_local clean_tests $(1):_MODULE=$(1)
...
ifneq (,$(value $(1).FILES))
	...
	$(CXX) $$(subst ../,,$$($(1).TMPFILES)) -L$(_LIBDIR) $$($(1).LDFLAGS) $(LDFLAGS) -o $(_EXEDIR)/$(1)
endif
$$($(1).SUBDIRS)::
	+$(MAKE) -C $$@
endef
...
$(_EXEDIR)/lib$(1)$(_LIBEXT): $(_MKFILES)  $$($(1).SUBDIRS) $$($(1).DEPS) $$($(1).TMPFILES)
	@printf " %b* $(link) [$(1)]\t$$@\n" "$(_TABS)"
ifeq "$(link)" "static"
...
ifdef $($(_MODULE).SUBDIRS)
	...
	$(MAKE) $(addprefix tests/,$($(_MODULE).TESTS))
	$(SECHO) "TEST Suite:$(T)|----|$($(_MODULE).TESTS)"
	@set -e ; for file in $($(_MODULE).TESTS) ; do $(ECHO) "TEST Running:$$file" ; ./tests/$$file ; done
#}}}++++++++++++++++++++++++++++++++++++++++++++++++++++
...
#####################################################################################
tests/%: export CXXFLAGS+=-I$(TOP)/UnitTest/test_fmwk -I$(TOP)/UnitTest/test_fmwk/mockpp/include/ -I$(PROJ_TOP) -I$(TOP)
tests/%: export LDFLAGS+=-L$(TOP)/UnitTest/test_fmwk/mockpp/lib/ -Wl,-rpath=$(TOP)/UnitTest/test_fmwk/mockpp/lib/
tests/%: tests/%.cpp $(_OBJDIR)/tests/%.o
