...
{
	uint32 u32Tmp;
	memcpy(&u32Tmp,p_pValue, sizeof(u32Tmp));

	//on ARM unaligned int32 are read/write incorrectly using force cast *(int*)  
...
void DMAP_WriteUint32(void * p_pValue, const uint8 * p_pBuf, uint8 p_ucSize)
{
	...
	//on ARM unaligned int32 are read/write incorrectly using force cast *(int*)  
	u32Tmp = ((uint32)*(p_pBuf++)) << 24;
	u32Tmp |= ((uint32)*(p_pBuf++)) << 16;
	u32Tmp |= ((uint32)*(p_pBuf++)) << 8;
	u32Tmp |= ((uint32)*(p_pBuf++));

	memcpy(p_pValue, &u32Tmp, sizeof(u32Tmp));
}
...
void DMAP_ReadContractMeta(const void * p_pValue, uint8 * p_pBuf, uint8* p_ucSize)
{
  ...
  *(p_pBuf++) = (uint8)(MAX_CONTRACT_NO);       // low byte
  
	uint16 u16ContractNo = g_oDmoContractsMap.size();
  // insert current entries number
  *(p_pBuf++) = u16ContractNo >> 8;
  *(p_pBuf++) = u16ContractNo;
...
const uint8 * DMAP_ExtractUint32( const uint8 * p_pData, uint32 * p_pulValue)
{
	...
	u32Tmp = ((uint32)*(p_pData++)) << 24;
	u32Tmp |= ((uint32)*(p_pData++)) << 16;
	u32Tmp |= ((uint32)*(p_pData++)) << 8;
	u32Tmp |= *(p_pData++);
	memcpy(p_pulValue, &u32Tmp, sizeof(u32Tmp));
