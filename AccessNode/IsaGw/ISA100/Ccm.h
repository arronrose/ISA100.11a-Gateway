...
public:
	uint16_t AuthEncrypt( const uint8_t *p_key, const uint8_t *p_nonce
			    ...
			    , uint8_t *p_out) ;
	bool CheckAuthDecrypt( const uint8_t* p_key, const uint8_t* p_nonce,
                                uint8_t* p_aad, uint16_t p_aadSz,
                                uint8_t* p_msg, uint16_t p_msgSz,
                                uint8_t* &p_out) ;
	void SetAuthenFldSz( uint16_t p_authFldSz ) { if(p_authFldSz<=16 && (!p_authFldSz || p_authFldSz>=4)) m_uiAuthFldSz = p_authFldSz ; }
	void SetLenFldSz( uint16_t p_lenFldSz ) { m_uiLenFldSz = p_lenFldSz ; }
	uint16_t GetAuthenFldSz( void ) { return m_uiAuthFldSz; }
	uint16_t GetLenFldSz( void ) { return m_uiLenFldSz; }
