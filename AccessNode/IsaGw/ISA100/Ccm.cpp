...
///////////////////////////////////////////////////////////////////////////////
uint16_t Ccm::AuthEncrypt( const uint8_t * p_key, const uint8_t * p_nonce
		    , uint8_t * p_aad, uint16_t p_aadSz
		    , uint8_t * p_msg, uint16_t p_msgSz
		    , uint8_t * p_out)
{
	if(!(	( m_uiLenFldSz )
		&&	( (p_aad && p_aadSz) || !p_aadSz )
		&&	( (p_msg && p_msgSz) || !p_msgSz )
		&&	( p_aadSz < 0xFF00 ) ) )	///< make sure that p_aadSz < 2^16 - 2^8, otherwise the p_aadSz encoding fails
	{	/// assert
		return 0;
	}
...
//
///////////////////////////////////////////////////////////////////////////////
bool Ccm::CheckAuthDecrypt( const uint8_t* p_key, const uint8_t* p_nonce,
				uint8_t* p_aad, uint16_t p_aadSz,
				uint8_t* p_encMsg, uint16_t p_encMsgSz,
				uint8_t* &p_out)
{
	if(!( 	( m_uiLenFldSz )
		&&	( (p_aad && p_aadSz) || !p_aadSz )
		&&	( p_encMsg && p_encMsgSz )	///< this contains the encrypted msg or the MIC or both; msg can't be null or zero length
		&&	( p_aadSz < 0xFF00 )		///< make sure that p_aadSz < 2^16 - 2^8, otherwise the p_aadSz encoding fails
		&&	( p_out && p_key && p_nonce )
		&& ( p_encMsgSz >= m_uiAuthFldSz ) ))///< the encrypted buffer also contains the U at the end
	{
		return false;
	}
...
///////////////////////////////////////////////////////////////////////////////
void Ccm::pushBlock0( uint16_t p_aadSz, uint16_t p_msgSz )
{
	m_uiWorkBufIdx = 0 ;
	/// Push Flags
	if(m_uiAuthFldSz){
		...
		m_prguiWorkBuf[m_uiWorkBufIdx++] = (p_aadSz?64:0) + 0 + (m_uiLenFldSz-1); // This is illegal in original CCM, but allowed in CCM*
	}
...
///////////////////////////////////////////////////////////////////////////////
void Ccm::pushAad( const uint8_t* p_aad, uint16_t p_aadSz )
{
	if ( !p_aadSz ) return ;
...
///////////////////////////////////////////////////////////////////////////////
void Ccm::pushMsg( const uint8_t* p_msg, uint16_t p_msgSz )
{
	if ( !p_msgSz ) return ;
...
///////////////////////////////////////////////////////////////////////////////
void Ccm::saltCtr( const uint8_t* /*p_msg*/, uint16_t p_msgSz )
{
