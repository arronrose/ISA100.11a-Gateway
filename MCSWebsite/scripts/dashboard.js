...
function RefreshDevice(idx, data){
	...
	DEVICES[idx].drawSet[6].attr({text:data.Manufacturer});	
	DEVICES[idx].drawSet[7].attr({text:data.Model});
	DEVICES[idx].drawSet[11].attr({text:data.ChannelName});
	DEVICES[idx].drawSet[12].attr({text:data.ReadingTime});
	DEVICES[idx].drawSet[13].attr({text:data.Value});
...
function DrawDevice(posX, posY, data){
	...
	var Address = paper.text(posX , posY + GAUGE_HEIGHT*8/100, deviceAddress);		
