...
function AddDeviceToDashboard(){
	...
	var minValue = document.getElementById("txtMinValue").value;
	var maxValue = document.getElementById("txtMaxValue").value ;  
	if (isNaN(minValue)){
		alert("Invalid value for Min value!")
		document.getElementById("txtMinValue").focus();
		return;
	} 
	if (isNaN(maxValue)){
	...
	AddDeviceInDashboard(slotNumber, deviceId, channelNo, gaugeType, minValue, maxValue)
	PopulateNoOfSlots();
