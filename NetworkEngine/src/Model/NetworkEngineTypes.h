...
struct ChannelDiagnostic {
        ...
        void toIndentString( std::ostringstream& stream) {
            stream << "{" << "S=" << std::hex << std::setw(2) << std::setfill('0') << (int) txSuccessful << ", CCA="
                        << std::hex << std::setw(2) << std::setfill('0') << (int) txCCABackoff << ", F=" << std::hex
                        << std::setw(2) << std::setfill('0') << (int) txFailed << "}";
            }
};
...
struct ChannelsPublicationData {
        ...
        ChannelsPublicationData(const Address64& publicationDeviceAddress_,
                    ChannelDiagnosticList& channelsDiagnosticsList_)
